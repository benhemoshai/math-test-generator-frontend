...
const FormCard = ({
  ...
  return (
    ...
    >
          ...
          <div className="bg-blue-50 p-3 sm:p-4 rounded-xl border border-blue-100">
            ...
            <div className="grid grid-cols-1 gap-2 sm:gap-3">
              {topics.slice(0, 3).map((topic) => (
                <label
                  key={topic}
                  className={`flex items-center space-x-2 rounded-lg px-2 sm:px-3 py-2 cursor-pointer transition-all duration-200 ${
                ...
                >
                  <input
                    type="checkbox"
                    className="form-checkbox h-4 w-4 sm:h-5 sm:w-5 accent-blue-600"
                    checked={selectedTopics.includes(topic)}
                    onChange={() => onTopicChange(topic)}
                  />
                  <span className={`text-xs sm:text-sm ${selectedTopics.includes(topic) ? 'font-medium text-blue-700' : 'text-gray-700'}`}>
            ...
            <div className="grid grid-cols-1 gap-2 sm:gap-3">
              {topics.slice(3, 5).map((topic) => (
                ...
                >
                  <input
                    type="checkbox"
                    className="form-checkbox h-4 w-4 sm:h-5 sm:w-5 accent-purple-600"
                    checked={selectedTopics.includes(topic)}
                    onChange={() => onTopicChange(topic)}
                  />
                  <span className={`text-xs sm:text-sm ${selectedTopics.includes(topic) ? 'font-medium text-purple-700' : 'text-gray-700'}`}>
                    {language === 'he' ? topicTranslations[topic] || topic : topic}
                  </span>
                </label>
              ))}
            ...
            <div className="grid grid-cols-1 gap-2 sm:gap-3">
              {topics.slice(5, 8).map((topic) => (
                ...
                >
                  <input
                    type="checkbox"
                    className="form-checkbox h-4 w-4 sm:h-5 sm:w-5 accent-indigo-600"
                    checked={selectedTopics.includes(topic)}
                    onChange={() => onTopicChange(topic)}
                  />
                  <span className={`text-xs sm:text-sm ${selectedTopics.includes(topic) ? 'font-medium text-indigo-700' : 'text-gray-700'}`}>
                    {language === 'he' ? topicTranslations[topic] || topic : topic}
                  </span>
                </label>
              ))}
      ...
      <div 
        className="flex items-center mb-4 sm:mb-6 bg-gray-50 p-2 sm:p-3 rounded-xl border border-gray-200 cursor-pointer hover:bg-gray-100 transition-colors"
        onClick={onMixToggle}
      >
        <div className="flex items-center gap-2 w-full">
          <div className="relative">
            <div className={`w-8 sm:w-10 h-4 sm:h-5 rounded-full transition-colors ${mixExams ? 'bg-purple-600' : 'bg-gray-300'}`}></div>
            <div className={`absolute top-0.5 left-0.5 bg-white w-3 h-3 sm:w-4 sm:h-4 rounded-full transition-transform ${mixExams ? 'translate-x-4 sm:translate-x-5' : ''}`}></div>
          </div>
          <span className="text-xs sm:text-sm font-medium text-gray-700">{translations.mixLabel}</span>
        </div>
      </div>
      ...
      <button
        ...
        className={`w-full py-2 sm:py-3 rounded-xl font-bold text-sm sm:text-base transition-all duration-300 ${
